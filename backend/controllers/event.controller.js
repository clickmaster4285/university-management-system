...
export const getAllEvents = handle(async (req, res) => {
  const { 
    type, 
    status, 
    category,
...
export const getEventById = handle(async (req, res) => {
...
export const getEventStats = handle(async (req, res) => {
  const total = await Event.countDocuments({ isDeleted: { $ne: true } }) || 0;
  const upcoming = await Event.countDocuments({ status: 'Upcoming', isDeleted: { $ne: true } }) || 0;
  const ongoing = await Event.countDocuments({ status: 'Ongoing', isDeleted: { $ne: true } }) || 0;
  const completed = await Event.countDocuments({ status: 'Completed', isDeleted: { $ne: true } }) || 0;
  const cancelled = await Event.countDocuments({ status: 'Cancelled', isDeleted: { $ne: true } }) || 0;
...
// Register for event
export const registerForEvent = handle(async (req, res) => {
  const { id } = req.params;
