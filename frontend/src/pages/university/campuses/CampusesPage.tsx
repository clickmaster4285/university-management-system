...
export function CampusesPage() {
  ...
  const fetchCampuses = async () => {
    try {
      ...
      toast.error(err?.message || "Failed to load campuses");
      setCampuses([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSetMain = async (campus: Campus) => {
    try {
      ...
      toast.error(err?.message || "Failed to set main campus");
    }
  };

  const statusBadge = (status: string) => {
    if (status === "Active") return <Badge className="bg-green-100 text-green-800 border-0">{status}</Badge>;
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            ...
            <p className="font-medium">
              {search ? "No campuses match your search" : "No campuses yet"}
            </p>
            {!search && (
              <Button
