...
export const Route = createFileRoute("/app/university")({
  head: () => ({
    meta: [
      { title: "Create University — ScholarOS" },
      { name: "description", content: "Register a new university in ScholarOS" },
    ],
  }),
    ...
    try {
      ...
      if (response.user && setUser) {
        const normalizedUser = {
          ...
          name:
            response.user.name ??
            (`${formData.firstName} ${formData.lastName}`.trim() || "University Admin"),
        };
